...
interface Query {
  _id: string;
  query: string;
  role: "farmer" | "officer";  // since your response shows "farmer" in role
  createdAt: string;
  updatedAt: string;
  geolocation: {
    lat: number;
...
const OfficerDashboard = () => {
  ...
  useEffect(() => {
    const fetchQueries = async () => {
      try {
        const res = await fetch("https://krishimitram-server.onrender.com/query/");
        if (!res.ok) {
                ...
                {queries.map((q) => (
                  <Card key={q._id} className="p-4">
                    <CardHeader className="flex justify-between items-center p-4">
                      <div>
                        <CardTitle className="mb-2">{q.userId.name} - {q.userId.address}</CardTitle>
                        <CardDescription><span className="font-bold text-black">Query :</span> {q.query}</CardDescription>
                      </div>
